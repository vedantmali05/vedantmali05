(function () {
    ...
    async function drawReportHeader(pdf, meta, orientation, options = {}) {
        ...
        const title = options.title || localStorage.getItem(keys.COMPANY_NAME) || "Coexio";
        const address = options.address || localStorage.getItem(keys.COMPANY_ADDRESS) || "";
        const subtitle = options.subtitle || "LOAD TEST REPORT";
        const logoPath = options.logoPath || localStorage.getItem(keys.COMPANY_LOGO) || "";
        const logoWidth = options.logoWidth || pt(54);
        const logoHeight = options.logoHeight || pt(54);
        ...
        function drawSectionBox(title, rows, x, y, width, boxOptions = {}) {
            const dryRun = boxOptions.dryRun || false;
            const targetHeight = boxOptions.targetHeight || null;
    ...
    function drawNoteBox(pdf, text, x, y, width, options = {}) {
        const paddingLeftRight = options.paddingX || 12;
        const paddingTopBottom = options.paddingY || 9;
        const fontSize = options.fontSize || 9;
        const lineSpacing = options.lineSpacing || 12;
        ...
        const stateKey = (options.state || "warn").toLowerCase();
    ...
    if (typeof window !== 'undefined') {
        ...
        window.drawReportHeader = drawReportHeader;
        window.drawNoteBox = drawNoteBox;
